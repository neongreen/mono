...
// TestMiseToolListCommonSettings tests the common settings functionality
func TestMiseToolListCommonSettings(t *testing.T) {
	// Create temporary home directory
	tempHome := t.TempDir()
	originalHome := os.Getenv("HOME")
	os.Setenv("HOME", tempHome)
	...
	// Create config with some common settings
	initialConfig := `[settings]
experimental = true
jobs = 4
