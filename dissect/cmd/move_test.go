...
`
	...
	t.Run("MoveTypeDeclaration", func(t *testing.T) {
		...
		sourceStr := string(sourceContent)
		if containsString(sourceStr, "type MyType struct") {
		...
		targetStr := string(targetContent)
		if !containsString(targetStr, "type MyType struct") {
			t.Errorf("MyType should be in target file")
		}

		// Check that comments are preserved
		if !containsString(targetStr, "MyType is a custom type") {
	...
	t.Run("MoveInterfaceDeclaration", func(t *testing.T) {
		...
		sourceStr := string(sourceContent)
		if containsString(sourceStr, "type MyInterface interface") {
		...
		targetStr := string(targetContent)
		if !containsString(targetStr, "type MyInterface interface") {
			t.Errorf("MyInterface should be in target file")
		}

		// Check that comments are preserved
		if !containsString(targetStr, "MyInterface defines behavior") {
			t.Errorf("Comments should be preserved in target file")
		}
	...
	t.Run("MoveMultipleTypesAndInterface", func(t *testing.T) {
		...
		if !containsString(targetStr, "type MyInt int") {
