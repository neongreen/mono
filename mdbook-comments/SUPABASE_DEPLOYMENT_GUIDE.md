...
The bundled `CommentForm` and `ReplyForm` components now post comments through the active backend adapter. Supabase integrations should implement `saveComment` and `saveReply` in the adapter (see `src/adapters/supabase.ts`) to enforce authentication and persist metadata—no manual overrides of `window.submitComment` or `window.submitReply` are required.
