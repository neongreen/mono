 ...
 * - Utility functions
 *
 * Backend adapters must implement the following interface:
 * {
 *   init: async () => Promise<void>,
 *   loadComments: async () => Promise<Comment[]>,
 *   saveComment: async (paragraphId, metadata, text, author) => Promise<Comment>,
 *   saveReply: async (parentCommentId, text, author) => Promise<Comment>,
 *   getCurrentAuthor: () => string | null,
 *   showAuthorInput: boolean,
 *   onAuthChange: (callback: (user) => void) => void  // optional
 * }
 */
...
(function() {
     ...
     */
    async function init(backendAdapter) {
    ...
    async function loadComments() {
        try {
            allComments = await backend.loadComments();
     ...
     * Create comment form
     */
    function createCommentForm(paragraphId, parentCommentId = null) {
        const form = document.createElement('div');
        form.className = parentCommentId ? 'reply-form' : 'comment-form';

        const isReply = !!parentCommentId;
        const currentAuthor = backend.getCurrentAuthor ? backend.getCurrentAuthor() : '';

        // Show author input if backend requires it
        if (backend.showAuthorInput && !currentAuthor) {
    ...
    window.submitComment = async function(paragraphId) {
        ...
        let author = backend.getCurrentAuthor ? backend.getCurrentAuthor() : null;
        if (backend.showAuthorInput && !author) {
        ...
        try {
            const newComment = await backend.saveComment(paragraphId, metadata, text, author);
    ...
    window.submitReply = async function(commentId) {
        ...
        try {
            const newReply = await backend.saveReply(commentId, text, author);
     ...
     */
    function refreshAllCommentSections() {
        document.querySelectorAll('.comment-section').forEach(section => {
            const paragraphId = section.getAttribute('data-paragraph-id');
            if (paragraphId && section.style.display !== 'none') {
                section.remove();
                toggleComments(paragraphId);
            }
        });
