/**
 * mdbook-comments - Client-side comment system with Google Sheets backend
 * 
 * This script handles:
 * - Loading comments from Google Sheets
...
(function() {
    ...
    // Google Sheets configuration - REPLACE THESE WITH YOUR VALUES
    const GOOGLE_CONFIG = {
     ...
     */
    function init() {
        console.log('Initializing mdbook-comments with Google Sheets...');
    ...
    /**
     * Load comments using API key (read-only, no auth required)
     */
    async function loadCommentsWithApiKey() {
        if (!GOOGLE_CONFIG.apiKey) return;
        ...
        try {
            ...
            if (!response.ok) {
                console.log('API key read failed, auth will be required for reading');
                return;
            }
    ...
    async function loadComments() {
        if (!accessToken) {
            console.log('No access token, skipping comment load');
            return;
        }
    ...
    function setupEventListeners() {
        // Event listeners handled via inline onclick
    }
    ...
    window.submitComment = async function(paragraphId) {
        ...
        const comment = {
            ...
            author: currentUser.email || currentUser.name,
            text: text,
        ...
        try {
            ...
            currentPageComments.push({ paragraphId, comment: localComment, confidence: 1.0 });
    ...
    window.submitReply = async function(commentId) {
        ...
        const reply = {
            ...
            metadata: JSON.stringify(parentComment.metadata),
            author: currentUser.email || currentUser.name,
            text: text,
            created: new Date().toISOString(),
            parent_id: commentId
        ...
        try {
            ...
            const localReply = {
                id: reply.id,
                paragraph_id: reply.paragraph_id,
