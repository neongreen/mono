/**
 * mdbook-comments - Client-side comment system
 * 
 * This script handles:
...
(function() {
    ...
    function matchComments() {
        ...
        commentLinks.forEach(link => {
            ...
            const exactMatches = allComments.filter(c => 
                c.metadata && c.metadata.id === paragraphId && !usedComments.has(c.id)
            );
        ...
        commentLinks.forEach(link => {
            ...
            // Find comments that might match this paragraph
    ...
    function updateCommentCounts() {
        document.querySelectorAll('.comment-link-wrapper').forEach(wrapper => {
            ...
            if (count > 0 && CONFIG.showCommentCount !== false) {
    ...
    function displayOrphanedComments() {
        ...
        // Find a good place to insert orphaned comments section
    ...
    function setupEventListeners() {
        // Comment link clicks are handled by inline onclick for now
        // Could be improved to use event delegation
    }
    ...
    window.toggleComments = function(paragraphId) {
        ...
        // Insert after the comment link wrapper
    ...
    window.submitReply = async function(commentId) {
        ...
        try {
            const response = await fetch(`${CONFIG.apiUrl}/comments/${commentId}/reply`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
