...
func TestSortTasks_ByCreated(t *testing.T) {
	now := time.Now()
	tasks := []*Task{
		{TaskID: "tak-3", Title: "Third", CreatedAt: now.Add(2 * time.Second)},
		{TaskID: "tak-1", Title: "First", CreatedAt: now},
		{TaskID: "tak-2", Title: "Second", CreatedAt: now.Add(1 * time.Second)},
	}

	sortTasks(tasks, "created")

	if tasks[0].TaskID != "tak-1" {
		t.Errorf("Expected first task to be tak-1, got %s", tasks[0].TaskID)
	}
	if tasks[1].TaskID != "tak-2" {
		t.Errorf("Expected second task to be tak-2, got %s", tasks[1].TaskID)
	}
	if tasks[2].TaskID != "tak-3" {
		t.Errorf("Expected third task to be tak-3, got %s", tasks[2].TaskID)
	}
}
...
func TestSortTasks_ByID(t *testing.T) {
	now := time.Now()
	tasks := []*Task{
		{TaskID: "tak-3", Title: "Third", CreatedAt: now},
		{TaskID: "tak-1", Title: "First", CreatedAt: now},
		{TaskID: "tak-2", Title: "Second", CreatedAt: now},
	}
...
func TestSortTasks_ByTitle(t *testing.T) {
	now := time.Now()
	tasks := []*Task{
		{TaskID: "tak-1", Title: "Zebra", CreatedAt: now},
		{TaskID: "tak-2", Title: "Apple", CreatedAt: now},
		{TaskID: "tak-3", Title: "Banana", CreatedAt: now},
	}
...
func TestSortTasks_StableSorting(t *testing.T) {
	...
	now := time.Now()
	tasks := []*Task{
		{TaskID: "tak-1", Title: "Task A", CreatedAt: now},
		{TaskID: "tak-2", Title: "Task A", CreatedAt: now},
		{TaskID: "tak-3", Title: "Task B", CreatedAt: now},
	}
