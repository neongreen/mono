...
| remote segments | `<remote>/tk-v4/` | all v4 segments live here |
| segment header | `"tk_major": 4` | required; ingest rejects others |

The same remote is reused; v4 writes under a `tk-v4/` subdirectory inside the existing remote.
...
The new `task.relocate` event replaces the legacy "move then renumber" sequence. It atomically handles:
...
This prevents intermediate "moved but un-renumbered" states.
...
- `tk move <task> --to <project> [--keep|--auto|--force N] [--on-collision fail|auto]` → emit task.relocate event
...
- All events stored locally, then pushed under `<remote>/tk-v4/`.
...
| tk number set <task> <N> | renumber within same project (task.number.set) |
| tk move <task> --to <project> [--keep\|--auto\|--force N] [--on-collision fail\|auto] | atomic move + renumber (task.relocate) |
...
## Appendix: Changes from Earlier v4 Spec

| area | change | rationale |
|------|--------|-----------|
| Migration | Automatic in-place upgrade; no manual export/import | user convenience; one-binary experience |
| Remote layout | reuse same remote; v4 writes under tk-v4/ subdir | avoids re-configuration |
| Segment header | added tk_major: 4 | hard version guard |
| Rollback | tk admin rollback-v4 command | safe undo |
| Move event | new atomic task.relocate replaces task.project.set + task.number.set | prevents intermediate "moved but un-renumbered" states |
| Removed commands | dropped tk mv; keep tk move + tk number set only | simplify CLI |
| Migration behavior | binary performs schema upgrade + backfill automatically | no manual dance |
| External systems | deferred | not implemented yet |
| Collision display | defined as alias-N-<node6> | consistent, minimal |
