...
func TestSegmentRoundTrip(t *testing.T) {
	...
	// Create a project and task using v4 format (for sync testing with legacy event format)
	projectUID := seedProject(t, db, "tk")
	_ = seedTask(t, db, projectUID, "test task", 1)

	// Create a legacy format event for sync testing (simulating old data)
	...
	// Use legacy format task_id for sync compatibility test
	nodeID, err := db.GetOrCreateNodeID()
	if err != nil {
		t.Fatalf("failed to get node ID: %v", err)
	}
	taskID := fmt.Sprintf("tk-1-%s", nodeID)
...
// TestEventAndTaskIDFormats - removed - GenerateTaskID has been removed, prefix functionality deprecated

// TestPrefixSync - removed - prefix functionality has been removed
